...
All tests in this module require CUDA as AMP is a GPU-specific feature.
"""
...
@pytest.fixture
def target_256(gpu_device):
    """Create a fixed target for 256x256 training."""
    # Use fixed seed for reproducibility
    torch.manual_seed(42)
    target = torch.randn(1, 1, 256, 256, device=gpu_device)
...
@pytest.fixture
def target_512(gpu_device):
    """Create a fixed target for 512x512 training."""
    # Use fixed seed for reproducibility
    torch.manual_seed(42)
    target = torch.randn(1, 1, 512, 512, device=gpu_device)
    # Normalize to [0, 1] range
    target = (target - target.min()) / (target.max() - target.min())
    return target
...
class TestAMPNumericalStability:
    ...
    @pytest.mark.gpu
    def test_amp_convergence_100_iterations(self, gpu_device, target_256):
        """
        ...
        model_fp32 = ProgressiveDecoder(input_size=256, use_amp=False).to(gpu_device)

        torch.manual_seed(123)
        model_amp = ProgressiveDecoder(input_size=256, use_amp=True).to(gpu_device)
    ...
    @pytest.mark.gpu
    def test_amp_gradient_stability(self, gpu_device, target_256):
        """
        ...
        """
        model = ProgressiveDecoder(input_size=256, use_amp=True).to(gpu_device)
    ...
    @pytest.mark.gpu
    def test_amp_scaler_updates(self, gpu_device, target_256):
        """
    ...
    @pytest.mark.gpu
    def test_amp_loss_consistency(self, gpu_device, target_256):
        """
    ...
    @pytest.mark.gpu
    def test_amp_parameter_updates(self, gpu_device, target_256):
        """
...
class TestAMPDifferentSizes:
    ...
    @pytest.mark.gpu
    def test_amp_convergence_512(self, gpu_device, target_512):
        """
        ...
        model = ProgressiveDecoder(input_size=512, use_amp=True).to(gpu_device)
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
        scaler = torch.amp.GradScaler("cuda")
    ...
    @pytest.mark.gpu
    def test_amp_different_learning_rates(self, gpu_device, target_256):
        """
