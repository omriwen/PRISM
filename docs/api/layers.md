...
Module: layers.py
Purpose: Custom neural network layers and components for PRISM
...
```python
ConditionalBatchNorm(num_features: int, enable: bool = True, **kwargs: Any) -> None
```
...
```python
DecoderUnit(chan_in: int, chan_out: int, use_bn: bool = True, kernel_size: int = 4, stride: int = 2, padding: int = 1, is_last: bool = False) -> None
```
...
```python
EncoderUnit(chan_in: int, chan_out: int, use_bn: bool = True, kernel_size: int = 4, stride: int = 2, padding: int = 1, is_first: bool = False, is_last: bool = False) -> None
```
...
```python
ScaleSigmoid(scale: float = 0.01) -> None
```
...
```python
activation(act_type: str, inplace: bool = True) -> torch.nn.modules.module.Module
```
...
```python
get_activation(name: str, **kwargs: Any) -> torch.nn.modules.module.Module
```
...
```python
init_weights_kaiming(module: torch.nn.modules.module.Module, mode: str = 'fan_in', nonlinearity: str = 'relu') -> None
```
...
```python
init_weights_orthogonal(module: torch.nn.modules.module.Module, gain: float = 1.0) -> None
```
...
```python
init_weights_xavier(module: torch.nn.modules.module.Module, gain: float = 1.0) -> None
```
