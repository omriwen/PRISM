# prism.core.telescope

Telescope aperture simulation.

This module implements realistic telescope aperture measurements with
Fraunhofer diffraction, noise, and various aperture configurations.
...
```python
Telescope(n: Optional[int] = None, r: Optional[float] = None, is_sum: Optional[bool] = None, sum_pattern: Optional[List[List[int]]] = None, cropping: Optional[bool] = None, obj_size: Optional[int] = None, snr: Optional[float] = None, propagator: Optional[torch.nn.modules.module.Module] = None, blur_image: Optional[bool] = None, aperture: Union[prism.core.apertures.Aperture, str, NoneType] = None, config: Optional[prism.core.telescope.TelescopeConfig] = None, **aperture_kwargs: Any) -> None
```

Simulates telescope aperture measurements with optical propagation.

The Telescope class models the measurement process of an astronomical telescope:
1. Object → propagate to k-space (using configurable propagator)
2. Apply aperture mask (circular, hexagonal, or obscured)
3. Propagate back to image plane
4. Take intensity (|·|²)
5. Add shot noise (optional)

This is the fundamental building block for the PRISM algorithm, simulating
realistic measurements through sparse telescope apertures in the Fourier plane.

Now inherits from both Instrument (multi-instrument framework) and nn.Module
(PyTorch integration), providing a unified interface while maintaining full
backward compatibility with existing PRISM code.

Attributes:
    n (Tensor): Image size in pixels
    r (Tensor): Aperture radius in pixels
    is_sum (Tensor): Whether to sum multiple measurements
    sum_pattern: Pattern for grouping measurements
    cropping (bool): Whether to crop images
    obj_size (int or None): Object size for cropping
    snr (float or None): Signal-to-noise ratio in dB
    noise (nn.Module): Noise model (ShotNoise or Identity)
    propagator: Optional free-space propagator (FraunhoferPropagator,
        AngularSpectrumPropagator, or other compatible propagator)
    blur_kernel (Tensor): Optional blur kernel for detector PSF
    blur_image (bool): Whether to apply blur
    aperture (Aperture): Aperture mask generator (strategy pattern)

Properties:
    x, y: Coordinate grids centered at image center

Methods:
    mask(center, r): Generate aperture mask using strategy pattern
    mask_batch(centers, r): Generate multiple masks (vectorized)
    forward(tensor, centers, ...): Complete measurement pipeline
    propagate_to_kspace(tensor, crop_size): Propagate to k-space
    propagate_to_spatial(tensor, crop_size): Propagate to image plane
    set_max_mean(tensor, center): Set normalization based on measurement
    compute_psf(): Compute point spread function (Instrument interface)
    resolution_limit: Theoretical resolution limit (Instrument interface)

Example:
    >>> # Default (Fraunhofer) propagation
    >>> telescope = Telescope(n=1024, r=50, snr=40)
    >>> measurement = telescope(object_image, centers=[[0, 0], [10, 10]])
    >>>
    >>> # With configuration object (new style)
    ...
    ...     snr=40
    ... )
    >>> telescope = Telescope(config=config)
    >>>
    >>> # With Angular Spectrum propagation (near-field)
    >>> from prism.core.grid import Grid
    >>> from prism.core.propagators import AngularSpectrumPropagator
    >>> grid = Grid(nx=1024, dx=10e-6, wavelength=520e-9)
    >>> prop = AngularSpectrumPropagator(grid, distance=0.1)
    >>> telescope = Telescope(n=1024, r=50, snr=40, propagator=prop)
    >>>
    >>> # Hexagonal aperture (JWST-style)
    >>> telescope = Telescope(n=1024, r=50, snr=40, aperture='hexagonal')
    >>>
    >>> # Obscured circular (VLT-style)
    >>> telescope = Telescope(
    ...     n=1024, r=50, snr=40, aperture='obscured',
    ...     inner_radius=10, spider_width=2, n_spiders=4
    ... )

Physics Notes:
    - Fraunhofer diffraction: Valid when Fresnel number << 1 (far-field)
    - Angular Spectrum: Exact propagation valid for all distances
    - k-space: Fourier domain where aperture mask is applied
    - Intensity: |propagate(propagate(object) * mask)|²
    - Shot noise: Gaussian approximation of Poisson photon statistics
    - Aperture shape affects PSF: circular → Airy disk, hexagonal → hex pattern

Propagator Types:
    - FraunhoferPropagator: FFT-based, fastest, far-field (F << 0.1)
    - AngularSpectrumPropagator: Exact, all distances, near-field support

Aperture Types:
    - 'circular': Simple circular opening (default, fastest)
    - 'hexagonal': Hexagonal segments (JWST, GMT, TMT)
    - 'obscured': Cassegrain with central obscuration and spiders (VLT, Hubble)
...
Supports both old-style (individual parameters) and new-style (config object)
initialization for backward compatibility.

Args:
    n (int, optional): Image size in pixels (old style)
    r (float, optional): Aperture radius in pixels (old style)
    is_sum (bool, optional): Whether to sum measurements (old style)
    sum_pattern: Pattern for grouping measurements (optional)
    cropping (bool, optional): Whether to crop input images (old style)
    obj_size (int or None): Size for cropping reconstructed object
    snr (float or None): Signal-to-noise ratio in dB (None for noiseless)
    propagator: Optional propagator instance (FraunhoferPropagator,
        AngularSpectrumPropagator, or compatible). If None, uses direct FFT.
    blur_image (bool, optional): Whether to apply detector blur (old style)
    aperture: Aperture instance or type string ('circular', 'hexagonal', 'obscured')
    config (TelescopeConfig, optional): Configuration object (new style)
...
Process:
1. FFT to k-space (Fraunhofer diffraction)
2. For each aperture center: apply mask, IFFT, take intensity, add noise
3. Optionally sum measurements according to pattern
4. Optionally apply detector blur

Args:
    tensor (Tensor): Input object image [B, C, H, W]
    centers: Aperture centers as Tensor or [[y0, x0], [y1, x1], ...]
            Can be from patterns.create_patterns() or list of lists.
            Defaults to [[0, 0]]
    r (float or None): Aperture radius. Defaults to self.r
    is_sum (bool or None): Whether to sum measurements. Defaults to self.is_sum
    sum_pattern: Grouping pattern for measurements. Defaults to self.sum_pattern
    add_noise (bool): Whether to add shot noise. Defaults to False

Returns:
    Tensor: Measurement(s) [N, 1, H, W] where N depends on is_sum and sum_pattern

Example:
    >>> measurement = telescope(object, centers=[[0, 0], [10, 10]], add_noise=True)
...
```python
TelescopeConfig(wavelength: float = 5.5e-07, n_pixels: int = 1024, pixel_size: float = 6.5e-06, grid_size: Optional[float] = None, aperture_diameter: Optional[float] = None, aperture_radius_pixels: float = 10, focal_length: Optional[float] = None, is_sum: bool = True, sum_pattern: Optional[List[List[int]]] = None, cropping: bool = True, obj_size: Optional[int] = None, snr: Optional[float] = 100, blur_image: bool = False, aperture_type: Optional[str] = None, aperture_kwargs: Optional[dict] = None) -> None
```

Configuration for telescope systems.

Attributes:
    aperture_diameter: Physical aperture diameter (for resolution calculations)
    aperture_radius_pixels: Aperture radius in pixels (for mask generation)
    focal_length: Telescope focal length (optional, for physical calculations)
    is_sum: Whether to sum measurements from multiple apertures
    sum_pattern: Pattern for grouping measurements
    cropping: Whether to crop input images
    obj_size: Size for cropping reconstructed object
    snr: Signal-to-noise ratio in dB (None for noiseless)
    blur_image: Whether to apply detector blur
